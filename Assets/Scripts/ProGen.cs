...
public class ProGen : MonoBehaviour
{
    ...
    private void BuildDataStructure()
    {
        ...
        foreach (Floor floor in _floors)
        {
            ...
            if (m_RandomizerRows)
                intialRows = UnityEngine.Random.Range(1, m_Rows);
            
            if (m_RandomizerColumns)
                initialColumns = UnityEngine.Random.Range(1, m_Columns);
        }
    }
            ...
            for (int row = 0; row < floor.Rows; row++)
            {
                for (int column = 0; column < floor.Columns; column++)
                {
                    ...
                    if (floor.FloorNumber == 0)
                        RoomPlacement(UnityEngine.Random.Range(0.0f, 1.0f) <= m_DoorPercentChance ? m_doorPrefab : m_wallPrefab, room, roomGO);
                    else
                    {
                        // rule: if window coverage percent is within
                        // threshold add a window otherwise add a basic wall
                        if (UnityEngine.Random.Range(0.0f, 1.0f) <= m_WindowPercentChance)
                        {
                            if (m_randomizeWindowSelection)
                            {
                                int windowIndex = UnityEngine.Random.Range(0, m_windowPrefabs.Length);
    ...
    private void RoomPlacement(GameObject prefab, Room room, GameObject roomGO)
    {
        ...
        if (room.HasRoof)
        {
            ...
            {
                int roofIndex = UnityEngine.Random.Range(0, m_rootPrefabs.Length);
